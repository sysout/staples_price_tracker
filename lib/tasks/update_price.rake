...
namespace :update_price do
  ...
  task :start, [:arg1] => :environment do |t, args|
    ...
    Product.joins(:alerts).where(
        ...
        date: args[:arg1].to_i.minutes.ago).distinct.order("products.alerts_count DESC").each do |product|
      ...
      ActiveRecord::Base.transaction do
        begin
          ...
          product.touch
          product.save!
          logger.info "#{product.name} product.updated_at: #{product.updated_at}"
        rescue Exception => e
